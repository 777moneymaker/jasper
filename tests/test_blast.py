...
class BlastTests(unittest.TestCase):
    ...
    def test_database_init_wrong(self):
        with self.assertRaises(FileNotFoundError):
            ...
            blast.Database(source_dir=self.test_dir / Path("data/fasta_test_data/seqs.fasta"), name="test_db")

    def test_repair_ok(self):
        db = blast.Database(source_dir=Path(""), name="test_db")
        result = db._repair_fasta(self.test_dir / Path("data/fasta_test_data/seqs.fasta"))
        expected = """>seqs|1
ATGCTGATCG
>seqs|2
GACGGTACG"""
    ...
    def test_aggregate_wrong_dir(self):
        with self.assertRaises(FileNotFoundError):
            ...
            blast.Database(source_dir=Path(""), name="test_db")._aggregate(self.test_dir / Path("data/fasta_test_data/seqs.fasta"), Path("blast_input.fasta"))

    def test_create(self,):
        db, output = blast.Database(source_dir=self.test_dir / Path("data/fasta_test_data"), name="test_db").create()
        self.assertTrue(map(lambda p: (self.test_dir / Path(p)).exists(), "test_db.nhr", "test_db.nin", "test_db.nsq"))
        db.clear_files()
    ...
    def test_query_ok(self):
        ...
        res = db.query(self.test_dir / Path("data/fasta_test_data"), config={"task": "blastn-short"}, num_threads=1)
    ...
    def test_query_wrong_dir(self):
        with self.assertRaises(FileNotFoundError):
            db = blast.Database(source_dir=self.test_dir / Path("data/fasta_test_data"), name="test_db")
            db.create()
            db.query(self.test_dir / Path("data/fasta_test_data/seqs.fasta"), config={}, num_threads=1)

    def test_query_wrong_command(self):
        with self.assertRaises(subprocess.SubprocessError):
            db = blast.Database(source_dir=self.test_dir / Path("data/fasta_test_data"), name="test_db")
            db.create()
            db.query(self.test_dir / Path("data/fasta_test_data/"), blast_format="test_wrong_format", config={"task": "blastn-short"}, num_threads=1)
        db.clear_files()
